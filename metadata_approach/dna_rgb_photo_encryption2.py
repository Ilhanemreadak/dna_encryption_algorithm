...
def load_image():
    ...
    if file_path:
        ...
        img_info_label.config(text=f"Yüklenen görüntü boyutu: {img_shape[0]}x{img_shape[1]}x{img_shape[2]}")
...
def dna_encode(byte_data):
    ...
    for i in range(0, len(binary_str), 2):
        ...
        if bit_pair in reverse_rule:
            dna_encoded += reverse_rule[bit_pair]
        else:
...
def dna_decode(dna_string):
    ...
    # Tam 8'in katı değilse dolgu ekle
...
def encrypt_image():
    ...
    try:
        ...
        # Metadata sözlüğü oluştur - şifre çözme için önemli bilgileri saklar
        metadata = {
            'height': h,
            'width': w,
            'channels': c
        ...
        # Tüm şifrelenmiş kanalları ve meta verileri pickle ile dosyaya kaydet
        encrypted_data = {
            'metadata': metadata,
...
def decrypt_image():
    try:
        ...
        h, w, c = metadata['height'], metadata['width'], metadata['channels']
...
def change_dna_rule():
    try:
        global selected_rule, rule, reverse_rule
